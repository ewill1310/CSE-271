...
public class PointPlotter {
	/**
	 * A simple string that contains the prompt for the menu used
	 * in this program.
	 */
	 ...
	 */
	private static final String DistPrompt = 
			"Find a point with what distance from origin? ";
	 ...
	 */
	private static void menu(ArrayList<Point> ptList) {
		...
		while(menu != 3) {
			...
			case 2:
				...
				ArrayList<Point> distList = 
						PointProcessor.findAll(ptList, dist);
	...
	/**
	 * The main method that runs the point plotter program to permit
	 * the user to process points from a text file.
	 * 
	...
	public static void main(String[] args) {
		...
		ArrayList<Point> ptList = PointProcessor.readPointsFromFile(fileName);

		// Print the information loaded for now.
		System.out.printf("Loaded %d points from %s\n", 
				ptList.size(), fileName);

		// Next run the menu loop to let the user perform different operations
		menu(ptList);
	}
}
